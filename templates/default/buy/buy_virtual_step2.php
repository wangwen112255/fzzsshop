    ...
    <table class="ncc-table-style" nc_type="table_cart">
     ...
     <tr id="pd_panel">
        ...
        <?php if ($output['member_info']['available_rc_balance'] > 0) { ?>
            <div class="mt5 mb5"><label><input type="checkbox" class="vm mr5" value="1" name="rcb_pay">使用债权金（可用金额：<em><?php echo $output['member_info']['available_rc_balance'];?></em>元）</label></div>
        <?php } ?>
...
$(document).ready(function(){
    ...
    $('#submitOrder').on('click',function(){
    	...
    	if (($('input[name="pd_pay"]').attr('checked') || $('input[name="rcb_pay"]').attr('checked')) && $('#password_callback').val() != '1') {
    		showDialog('使用债权金/预存款支付，需输入支付密码并使用  ', 'error','','','','','','','','',2);
    		return;
    	}
    	SUBMIT_FORM = false;
    	$('#form_buy').submit();
