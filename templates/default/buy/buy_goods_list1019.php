  ...
  <table class="ncc-table-style">
    ...
    <tbody>
      ...
      <tr id="cart_item_<?php echo $cart_info['cart_id'];?>" class="shop-list <?php echo ($cart_info['state'] && $cart_info['storage_state']) ? '' : 'item_disabled';?>">
        ...
        <td class="w120"><em><?php echo $cart_info['goods_price_zq']; ?></em></td>
        <td class="w120"><em><?php echo $cart_info['goods_service_price']; ?></em></td>
        ...
        <td class="w120"><?php if ($cart_info['state'] && $cart_info['storage_state']) {?>
          ...
          <td class="w120"><?php if ($cart_info['state'] && $cart_info['storage_state']) {?>
                  <em id="item<?php echo $cart_info['cart_id']; ?>_subtotalZQ" nc_type="eachGoodsTotalZQ"><?php echo $cart_info['goods_zq_total']; ?></em>
              <?php } elseif (!$cart_info['storage_state']) {?>
                  <span style="color: #F00;">库存不足</span>
              <?php }elseif (!$cart_info['state']) {?>
                  <span style="color: #F00;">已下架</span>
          ...
          <td class="w120"><?php if ($cart_info['state'] && $cart_info['storage_state']) {?>
                  <em id="item<?php echo $cart_info['cart_id']; ?>_subtotalService" nc_type="eachGoodsTotalService"><?php echo $cart_info['goods_service_total']; ?></em>
              <?php } elseif (!$cart_info['storage_state']) {?>
                  <span style="color: #F00;">库存不足</span>
              <?php }elseif (!$cart_info['state']) {?>
                  <span style="color: #F00;">已下架</span>
          ...
          <td class="w120"><?php if ($cart_info['state'] && $cart_info['storage_state']) {?>
                  <em id="item<?php echo $cart_info['cart_id']; ?>_subtotalTrue" nc_type="eachGoodsTotalTrue"><?php echo $cart_info['goods_total'] + $cart_info['goods_zq_total']; ?></em>
              <?php } elseif (!$cart_info['storage_state']) {?>
                  <span style="color: #F00;">库存不足</span>
              <?php }elseif (!$cart_info['state']) {?>
                  <span style="color: #F00;">已下架</span>
      ...
      <tr>
        <td class="tr" colspan="20"><div class="ncc-store-account">
            ...
            <dl>
              <dt>商品服务费：</dt>
              <dd><em id="eachStoreGoodsTotalService_<?php echo $store_id;?>"><?php echo $output['store_goods_total_service'][$store_id];?></em><?php echo $lang['currency_zh'];?></dd>
            </dl>

            <dl>
              <dt>商品债权金：</dt>
              <dd><em id="eachStoreGoodsTotalZQ_<?php echo $store_id;?>"><?php echo $output['store_goods_total_zq'][$store_id];?></em><?php echo $lang['currency_zh'];?></dd>
            </dl>
        ...
        <?php if (!empty($output['available_rcb_amount'])) { ?>
            <div class="mt5 mb5">
              <label>
                ...
                使用债权金（可用金额：<em><?php echo $output['available_rcb_amount'];?></em><?php echo $lang['currency_zh'];?>）</label>
            </div>
       <?php } ?>
      ...
      <?php } ?>
      <?php if (!empty($output['available_pd_amount']) && !empty($output['available_rcb_amount'])) { ?>
      <div class="mt5 mb5">如果二者同时使用，系统优先使用债权金&nbsp;&nbsp;</div>
      <?php } ?>
