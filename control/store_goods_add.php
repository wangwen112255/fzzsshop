...
class store_goods_addControl extends BaseSellerControl {
    ...
    public function save_goodsOp() {
        if (chksubmit()) {
            ...
            $common_array['goods_price_zq']   = floatval($_POST['g_price_zq']);
            $common_array['goods_service_price']   = floatval($_POST['g_price_service']);
            // 序列化保存手机端商品描述数据
            if ($_POST['m_body'] != '') {
            ...
            $common_array['transport_id']       = ($_POST['freight'] == '1') ? intval($_POST['transport_id']) : $_POST['freight'];  // 运费模板
            ...
            if ($common_id) {
                ...
                if (is_array($_POST['spec'])) {
                    foreach ($_POST['spec'] as $value) {
                        ...
                        $goods['goods_price_zq']   = floatval($common_array['goods_price_zq']);
                        $goods['goods_service_price']   = floatval($common_array['goods_service_price']);
