...
class predepositControl extends BaseMemberControl {
    ...
    public function rechargecard_addOp()
    {
        ...
        if (!$sn || strlen($sn) > 50) {
            showMessage('平台债权金卡号不能为空且长度不能大于50', '', 'html', 'error');
            exit;
        }

        try {
            model('predeposit')->addRechargeCard($sn, $_SESSION);
            showMessage('平台债权金使用成功', urlShop('predeposit', 'rcb_log_list'));
        } catch (Exception $e) {
	...
	private function profile_menu($menu_type,$menu_key=''){
		...
		switch ($menu_type) {
            ...
            case 'rechargecard_add':
                $menu_array[] = array('menu_key'=>'rechargecard_add','menu_name'=>'债权金充值','menu_url'=>'javascript:;');
                break;
			case 'log':
				break;
			case 'cashadd':
