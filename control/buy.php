...
class buyControl extends BaseBuyControl {
    ...
    public function buy_step1Op() {
        ...
        Tpl::output('store_goods_total_service', $result['store_goods_totalService']);
        Tpl::output('store_goods_total_true', $result['store_goods_totalTrue']);
        Tpl::output('store_goods_total_zq', $result['store_goods_totalZQ']);
    ...
    public function buy_step2Op() {
        ...
        if($result['msg'] == '您必须使用债券金购买')
        {
        	showMessage($result['msg'], 'index.php?act=predeposit&op=rcb_log_list', 'html', 'error');
        }
    ...
    public function payOp() {
        ...
        foreach ($order_list as $key => $order_info) {
            ...
            $payed_amount = 0;//floatval($order_info['rcb_amount'])+floatval($order_info['pd_amount']);
            //计算相关支付金额
            ...
            if ($order_info['payment_code'] == 'offline') {
                ...
                if ($payed_amount > 0) {
                    ...
                    if (floatval($order_info['rcb_amount']) > 0) {
                        $payed_tips = '债权金已支付：￥'.$order_info['rcb_amount'];
                    }
    ...
    public function zs_payOp() {
        ...
        //查询支付单信息
        $model_order= Model('zq');
		
        $zs_info = $model_order->getZsInfo(array('pay_id'=>$pay_sn,'user_id'=>$_SESSION['member_id']));
		
        if(empty($zs_info)){
            showMessage(Language::get('para_error'),'','html','error');
        }
        if (intval($zs_info['is_pay'])) {
            showMessage('您的订单已经支付，请勿重复支付','index.php?act=predeposit','html','error');
        }
        Tpl::output('zs_info',$zs_info);
