...
class store_goods_onlineControl extends BaseSellerControl {
    ...
    public function edit_save_goodsOp() {
        ...
        $update_common['goods_price_zq']   = floatval($_POST['g_price_zq']);
        $update_common['goods_service_price']   = floatval($_POST['g_price_service']);
        ...
        $update_common['transport_id']       = ($_POST['freight'] == '1') ? intval($_POST['transport_id']) : $_POST['freight']; // 运费模板
        ...
        if (is_array ( $_POST ['spec'] ))
        {
            foreach ($_POST['spec'] as $value) {
                ...
                if (!empty($goods_info))
                {
                    ...
                    $update['goods_price_zq']   = floatval($_POST['g_price_zq']);
                    $update['goods_service_price']   = floatval($_POST['g_price_service']);
                    // 虚拟商品不能有赠品
                ...
                else
                {
                    ...
                    $insert['goods_price_zq']   = floatval($_POST['g_price_zq']);
                    $insert['goods_service_price']   = floatval($_POST['g_price_service']);
        ...
        if ($return) {
            //提交事务
            Model()->commit();
            showDialog("ccc成功".$_POST['freight'], $_POST['ref_url'], 'succ');
        } else {
            //回滚事务
            Model()->rollback();
            showDialog(L('store_goods_index_goods_edit_fail'), urlShop('store_goods_online', 'index'));
