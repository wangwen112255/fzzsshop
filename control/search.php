...
class searchControl extends BaseHomeControl {
    ...
    public function indexOp() {
        ...
        if($default_classid == 1135 && $_SESSION['is_vip'] != 1)
        {
            showDialog('您不是VIP会员');
        }
        ...
        if (intval($_GET['cate_id']) > 0) {
            ...
            $goodsClassInfo = $model_goods_class->getGoodsClassInfoById($_GET['cate_id']);
            
            if($goodsClassInfo['gc_parent_id'] == 1135 && $_SESSION['is_vip'] != 1)
            {
                showDialog('您不是VIP会员');
            }
