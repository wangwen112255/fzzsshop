...
class paymentControl extends BaseHomeControl{
	 ...
	 * 债事备案费用
	 */
	public function zs_orderOp(){
	    $zs_sn = $_POST['zs_sn'];
	    $payment_code = $_POST['payment_code'];
	    $url = 'index.php?act=predeposit';

	    if(!preg_match('/^\d{18}$/',$zs_sn)){
	        showMessage('参数错误',$url,'html','error');
	    }
        ...
        $result = $logic_payment->getZsOrderInfo($zs_sn,$_SESSION['member_id']);

        if(!$result['state']) {
            showMessage($result['msg'], $url, 'html', 'error');
        }
        if ($result['data']['is_pay'] || empty($result['data']['api_pay_amount'])) {
            showMessage('该充值单不需要支付', $url, 'html', 'error');
        }
	...
	public function notifyOp(){
		...
		} elseif ($order_type == 'ZS_order') {

		    $result = $logic_payment->getZsInfo($out_trade_no);
		...
		if ($order_type == 'real_order') {
		    ...
		    $result = $logic_payment->updateZsOrder($out_trade_no,$trade_no,$payment_info,$order_pay_info);
		}
	...
	public function returnOp(){
		...
		if ($order_type == 'real_order') {
		    ...
		    $result = $logic_payment->getZsOrderInfo($out_trade_no);
		    if(!$result['state']) {
		        showMessage($result['msg'], $url, 'html', 'error');
		    }
		    if ($result['data']['is_pay'] == 1) {
		...
		if ($order_type == 'real_order') {
			...
			$pay_ok_url = SHOP_SITE_URL.'/index.php?act=member&op=home';
		}
