...
class member_securityControl extends BaseMemberControl {
    ...
    public function authOp() {
        ...
        if (chksubmit(false,true)) {
            if (!in_array($_POST['type'],array('modify_pwd','modify_mobile','modify_email','modify_membercard','modify_paypwd','pd_cash'))) {
            ...
            if (!in_array($_GET['type'],array('modify_pwd','modify_mobile','modify_email','modify_membercard','modify_paypwd','pd_cash'))) {
                redirect('index.php?act=member_security&op=index');
            }
     ...
     * 绑定会员卡
     */
    public function modify_membercardOp() {
    	$model_member = Model('member');
    	...
    	$obj_validate = new Validate();
    	$obj_validate->validateparam = array(
    			array("input"=>$_POST["member_card"],		"require"=>"true",		"message"=>'请输入要绑定的会员卡号'),
    	);
    	$error = $obj_validate->validate();
    	if ($error != ''){
    		showValidateError($error);
    	}
    	$update	= $model_member->editMember(array('member_id'=>$_SESSION['member_id']),array('member_card'=>trim($_POST['member_card'])));
    	if (!$update) {
                showDialog('系统发生错误，如有疑问请与管理员联系');
        }
         showDialog('会员卡绑定成功','index.php?act=member_security&op=index','succ');
    }
